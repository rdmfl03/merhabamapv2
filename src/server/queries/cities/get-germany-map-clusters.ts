...
export type GermanyMapClustersSummary = {
  clusters: GermanyMapCluster[];
  placeCount: number;
  eventCount: number;
};

export async function getGermanyMapClustersSummary(): Promise<GermanyMapClustersSummary> {
...
export async function getGermanyMapClusters(): Promise<GermanyMapCluster[]> {
  const result = await getGermanyMapClustersSummary();
