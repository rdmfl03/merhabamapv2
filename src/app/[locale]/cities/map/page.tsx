...
export default async function CitiesMapPage({ params, searchParams }: MapPageProps) {
  ...
  const [t, placesTexts, eventsTexts, pageData] = await Promise.all([
  ...
  const cardReturnPath = isNational
    ? `/${locale}/cities/map`
    : `/${locale}/cities/map?city=${pageData.city.slug}`;

  const collectionPath = isNational ? "/cities/map" : `/cities/map?city=${pageData.city.slug}`;
